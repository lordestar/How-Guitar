...
CHORD_DATABASE['Gm'] = [
  {
    ...
    strings: [
      { string: 0, fret: 3, note: 'G', label: '3' },
      { string: 1, fret: 5, note: 'D', label: '5' },
      { string: 2, fret: 5, note: 'G', label: '5' },
      { string: 3, fret: 3, note: 'Bb', label: '3' },
      { string: 4, fret: 3, note: 'D', label: '3' },
      { string: 5, fret: 3, note: 'G', label: '3' },
...
function noteToSemitone(note) {
  var idx = ROOT_NOTES.indexOf(note);
...
function getChordNotes(root, type) {
  if (!type) type = 'major';
  var intervals = CHORD_TYPES[type];
  if (!intervals) return { error: '不支持的和弦类型: ' + type };
  var rootIdx = noteToSemitone(root);
  ...
  return {
    ...
    notes: notes,
    noteValues: intervals.map(function(semi) { return (rootIdx + semi) % 12; }),
  };
}

// 从数据库获取和弦指法
function getChordFromDatabase(root, type) {
  var chordName = root + (type === 'major' || type === '' ? '' : type);
  // 特殊处理
